...
public class AmqpListeners {
    ...
    @RabbitListener(queues = QueueConstants.QUE_UC_TASK_FLOW_START)
    public void taskFlowStartListener(TaskFlowReqDTO reqDTO) throws JsonProcessingException {
        ...
        // 将理赔车任务放入有序队列中
        redisTemplate.opsForZSet().add(RedisConstants.ZsetKeyConstants.ZSET_UC_AUTO_ALLOT, objectMapper.writeValueAsString(reqDTO),
                score);
    }

    @RabbitListener(queues = QueueConstants.QUE_UC_TASK_FLOW_SUBMIT)
    public void taskFlowSubmitListener() {

    }

    @RabbitListener(queues = QueueConstants.QUE_UC_ALLOT_OPERATOR)
